...
```python
{'exp':xxx, # Expiration Time 此token的过期时间的时间戳   time.time()+5*60
 'iss':xxx，# (Issuer) Claim 指明此token的签发者   
 'aud':xxx, #(Audience) Claim 指明此token的签发群体  'ios'  ios小组门票请进入
...
​		因为ajax只能向同源服务器发出数据请求所以必须用到CORS跨域资源共享
...
| 响应头                           | 作用                                                         | 备注 |
| -------------------------------- | ------------------------------------------------------------ | ---- |
| Access-Control-Allow-Origin      | 服务器接受得域       1.服务器白名单：接受谁可以进            |      |
...
| Access-Control-Allow-Methods     | 告诉浏览器，服务器接受得跨域请求方法 (GET,POST,PUT)          | 必选 |
...
```
#将移动包转至虚拟机主目录
#执行  tar -zxvf django-cors-header-3.0.0.tar.gz
#cd django-cors-headers-3.0.2
#sudo python3 setup.py install
#查看版本  pip3 freeze|grep 'cors'  #注意是 3.0.2的cors
#查看Django版本是否被改变 pip3 freeze|grep 'Django'  #1.11.8 的Django
```
...
## 4.3 如何设计符合RESTful 特征的API   （视图绑定的url）
...
- DELETE（DELETE）：从服务器删除资源。
  ...
  ​			JsonResponse({'code':200,  'data': {}, 'error': 'The username is already existed~~'})

  比如内部状态码： 200 成功    所有数据放在data字段里面
...
- 开发 ‘同一时间只有一个用户可登录’功能时，发现同一个浏览器中多个标签在多次登录后，旧标签

  页面里校验权限功能均无法使用；最终发现前端页面再发送token时，使用的时js变量中的token;此token在用户进入页面时就已经进行了取值，导致该token有一定的滞后性,最终在每次发送token前，再次获取一下当前的本地存储中的
...
​					http://127.0.0.1:8000/asdsknfjdsnfj

​				2.发送

注册一个<容联 云通讯>可调试能给手机号发短信   Api 把参数写好 在视图函数中对外通信 process 逻辑编辑网站
